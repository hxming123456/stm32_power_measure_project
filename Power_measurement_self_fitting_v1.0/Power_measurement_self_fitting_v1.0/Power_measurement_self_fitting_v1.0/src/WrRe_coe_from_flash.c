...
void make_coe_to_data(uint16_t *data)
{
	uint16_t tmp_int = 0;
	uint16_t tmp_dec = 0;
	double tmp_coea_p = self_adjust_coea_p;
	double tmp_coeb_p = self_adjust_coeb_p;
	double tmp_coer_p = self_adjust_coer_p;
...
void pars_coe_from_data(uint16_t *data)
{
	...
	if(data[0]==0)
	{
		self_adjust_coea_p = tmp_int+tmp_dec;
	}
	else
	{
		self_adjust_coea_p = -(tmp_int+tmp_dec);
	...
	if(data[3]==0)
	{
		self_adjust_coeb_p = tmp_int+tmp_dec;
	}
	else
	{
		self_adjust_coeb_p = -(tmp_int+tmp_dec);
	...
	if(data[6]==0)
	{
		self_adjust_coer_p = tmp_int+tmp_dec;
	}
	else
	{
		self_adjust_coer_p = -(tmp_int+tmp_dec);
