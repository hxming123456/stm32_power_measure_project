    ...
    def open_uart(self,event):
        try:
            if self.uart_open_flag == 0:
                try:
                    self.ser = serial.Serial(self.load_file_com[self.serial_com_choise.GetSelection()],9600,bytesize=8, parity='N', stopbits=1, timeout=0.01, writeTimeout=0, xonxoff=0,rtscts=0)
                    self.ac6530_ser = serial.Serial(self.load_file_com[self.ac6530_com_choise.GetSelection()], 9600,bytesize=8, parity='N', stopbits=1, timeout=0.01, writeTimeout=0,xonxoff=0, rtscts=1)
    ...
    def data_parsing(self,data):
        ...
        if data == 'p':
          ...
          #  self.ac6530_ser.flush()
            ...
            print "stm32 data paesing failed"
